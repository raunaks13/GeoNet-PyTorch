...
def init_weights(net, init_type='normal', gain=0.02):
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            ...
            init.uniform_(m.weight.data, 0.0, 1.0)
            init.constant_(m.bias.data, 0.0)
...
class _ShuffleUpBlock(nn.Module):
    def __init__(self, input_nc, up_scale, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.PReLU(),
                 use_bias=False):
...
class _UNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, layers=4, norm='batch', activation='PReLU', drop_rate=0,
                 add_noise=False, gpu_ids=[],
                 weight=0.1):
